...
t0 = time()
# Create data
n_prones = 100_000 if len(sys.argv) == 1 else int(sys.argv[1])
phones = """Iphone X
Iphone 8 Max
...
ser = pd.Series([phones[i] for i in np.random.randint(0, len(phones), (n_prones,))])
...
hshv = np.unpackbits(hsh.view(np.uint8))  # open a view into the continious memory, interpret as uint8
